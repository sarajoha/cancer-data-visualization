...
def get_county_attributes(svg_file_name):
    ...
    with open(svg_file_name, 'r') as svg_file:
        parsed_svg = minidom.parse(svg_file)

    fips_county = []
    for path in parsed_svg.getElementsByTagName('path'):
        fisp = str(path.getAttribute('id'))
        boundaries = str(path.getAttribute('d'))
        fips_county.append((fisp, boundaries))
...
def get_boundary_coordinates(boundary_data):
    ...
    """
    coordinates = []
    boundaries = boundary_data.split(' ')

    for item in boundaries:
        if (item == 'M') or (item == 'L') or (item == 'z') or (item == ''):
            pass
        else:
            coord = item.split(',')
            if 'M' in coord[1]:
                newcoord = list(coord[1])
                ycoord = newcoord[:-1]
                coord2 = "".join(ycoord)
                coordinates.append((float(coord[0]), float(coord2)))
            else:
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    with open(csv_file_name, 'w') as csvfile:
        ...
        for item in county_attributes:
            fisp = item[0]
            #Get a list of coordinates (tuples) from county_attributes
            boundary_coords = get_boundary_coordinates(item[1])
            #Get center coordinates (county_center) as a tuple
            coords = compute_county_center(boundary_coords)
            #Get x and y coordinates of county center
            x_coord = coords[0]
            y_coord = coords[1]
            #Write row to csv file, column beign FISP code, x-coord of center, y-coord of centers
            csvwriter.writerow([fisp, x_coord, y_coord])
